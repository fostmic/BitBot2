...
namespace BitBot
{
    internal static class MarketData
    {
        ...
        static private void loadOrderBook()
        {
            ...
            //    { }
            //}
            if (timeLastOrderBookLoadTry != new System.DateTime() && (System.DateTime.Now - timeLastOrderBookLoadTry).TotalSeconds < Properties.Settings.Default.cbMinTimeReload)
            {
                int secondsToSleep = Properties.Settings.Default.cbMinTimeReload - (int)(System.DateTime.Now - timeLastOrderBookLoadTry).TotalSeconds;
        ...
        internal static ProcessWebsocketMessageResponse ProcessWebsocketMessage(string message, long lastSeqNum = long.MaxValue, bool ignoreReload = false)
        {
            ...
            if (Properties.Settings.Default.cbMessageTrigger && (jsonMessage.sequence - firstCoinbaseSequenceNumber) % Properties.Settings.Default.cbAutoReloadMessages == 0 && (jsonMessage.sequence - firstCoinbaseSequenceNumber) != 0)
            {
                statusMsg = System.DateTime.Now.ToString(DATETIME_FORMAT_STRING) + " Order book reload triggered due to " 
                    + Properties.Settings.Default.cbAutoReloadMessages + " websocket messages received since last reload.  Sequence = " + jsonMessage.sequence;
                retVal = ProcessWebsocketMessageResponse.timeToReloadOrderBook;
            }
            else if (Properties.Settings.Default.cbTimeTrigger && (System.DateTime.Now - timeLastOrderBookLoadTry).TotalSeconds > Properties.Settings.Default.cbAutoReloadTime)
            {
                statusMsg = System.DateTime.Now.ToString(DATETIME_FORMAT_STRING) + " Order book reload triggered due to "
                    + Properties.Settings.Default.cbAutoReloadTime + " seconds elapsed since last reload.  Sequence = " + jsonMessage.sequence;
                retVal = ProcessWebsocketMessageResponse.timeToReloadOrderBook;
            }
